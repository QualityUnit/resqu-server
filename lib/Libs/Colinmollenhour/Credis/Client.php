...
namespace Resque\Libs\Colinmollenhour\Credis;
...
class Client {
    ...
    {
    	[$command, $channel, $subscribedChannels] = $this->__call('punsubscribe', func_get_args());
    ...
    public function pSubscribe($patterns, $callback)
    {
        ...
        while ($patternCount--) {
            if (isset($status)) {
                [$command, $pattern, $status] = $this->read_reply();
            } else {
    ...
    {
    	[$command, $channel, $subscribedChannels] = $this->__call('unsubscribe', func_get_args());
    	$this->subscribed = $subscribedChannels > 0;
    	return array($command, $channel, $subscribedChannels);
    }
    ...
    public function subscribe($channels, $callback)
    {
        ...
        while ($channelCount--) {
            if (isset($status)) {
                [$command, $channel, $status] = $this->read_reply();
            } else {
